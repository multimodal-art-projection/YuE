...
def _private_flash_attention_available(config, fused):
    ...
    # ROCm exposes CUDA-shaped private operators, but its packed/varlen path
    # does not support the ``seqused_k`` contract used by ``_decode``.
...
class GraphAR:
    ...
    def __init__(self, model, prefixes, max_tokens, *, capture=True, attention_backend="auto", fuse_projections=False):
        ...
        # HIP never selects flash or cuDNN; public masked SDPA is the fallback.
